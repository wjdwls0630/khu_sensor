...
"""@package docstring
@brief	case 2 :
LPF cut_off frequency = 250Hz, sampling frequency = 1.5kHz
...
if __name__ == "__main__":
    ...
    """
    @brief	case 2 : 
    LPF cut_off frequency = 250Hz, sampling frequency =23kHz
    """
    # create signal
    signal = SignalJin(0, 0.015, fs=1.5e3)
    ...
    ax.annotate(rf'$f_cL$'+'={}Hz'.format(lpf.get_fcL()),
                ...
                xytext=(lpf.get_fcL()+150, 0.5), textcoords='data', arrowprops=dict(arrowstyle="->", color='r'))
    ...
    ax.annotate('Nyquist rate'+"\n"+r'$\frac{f_s}{2}$'+"={}kHz".format(lpf.get_fs()/2e3),
                ...
                xytext=(lpf.get_fs()/2+150, 0.5), textcoords='data', arrowprops=dict(arrowstyle="->", color='g'))
    ...
    ax.annotate(rf'$f_cL$' + '={}Hz'.format(lpf.get_fcL()),
                ...
                xytext=(lpf.get_fcL() + 150, 20*numpy.log10(1/numpy.sqrt(2))), textcoords='data', arrowprops=dict(arrowstyle="->", color='r'))
    ax.axvline(x=lpf.get_fs() / 2, color='g', linestyle='--')
    ax.annotate('Nyquist rate'+"\n"+r'$\frac{f_s}{2}$'+"={}kHz".format(lpf.get_fs()/2e3),
                xy=(lpf.get_fs() / 2, -40), color='g', xycoords='data',
                xytext=(lpf.get_fs() / 2 + 150, -40), textcoords='data', arrowprops=dict(arrowstyle="->", color='g'))
    ax.set_xlabel(r'$f$ (Hz)')
    ...
    pyplot.savefig("../data/LPF/LPF2.png")
    pyplot.show()
