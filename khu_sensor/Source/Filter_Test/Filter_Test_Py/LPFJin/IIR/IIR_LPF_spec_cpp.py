...
if __name__ == "__main__":
    ...
    # get coefficient
    with open("../../Data/Singnal_CPP/Coefficient/IIR_LPF_Coefficient.txt", 'r') as dataFile:
        line = dataFile.readline()  # read one line - A_CoEfficient
        Header = line.split(" ")
        A_Coef = []
        for i in range(2, len(Header) - 1, 1):
            if float(Header[i]) != 0.0 :
                A_Coef.append(float(Header[i]))

        line = dataFile.readline()  # read one line - B_CoEfficient
        Header = line.split(" ")
        B_Coef = []
        for i in range(2, len(Header) - 1, 1):
            if i != 2:
                B_Coef.append(-float(Header[i]))
            else :
                B_Coef.append(float(Header[i]))
    ...
    B_Coef.reverse()
    """
    num = numpy.array(A_Coef)
    den = numpy.array(B_Coef)
    h_lpf_n = tf(num, den)
    #pzmap(h_lpf_n)
    print(h_lpf_n)
