...
"""@package docstring
@brief	case 2 : (various frequency included) signal with SNR 116
...
if __name__ == "__main__":
    ...
    # create signal
    with open("../../../Data/IIR/Signal/Signal_CPP_2.txt", 'r') as dataFile:
        line = dataFile.readline()  # read one line - frequency
        Header = line.split(" ")
        signal_frequency = []
        for i in range(2, len(Header) - 1, 1):
            signal_frequency.append(float(Header[i]))
    ...
    ax.set(title="signal_cpp_2")
    ax.plot(n, value, 'k', label=rf'Signal_CPP_2')
    ...
    ax.grid()
    ax.legend()
    pyplot.savefig("../../../Data/IIR/Signal/Signal_CPP_2.png")
