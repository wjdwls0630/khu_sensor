...
"""@package docstring
@brief	case 2 : signal with noise which SNR is -10dB
...
if __name__ == "__main__":
    ...
    ax.annotate('Nyquist rate'+"\n"+r'$\frac{f_s}{2}$'+"={}kHz".format(signal.get_fs()/2e3),
                xy=(signal.get_fs()/2, 0.025), color='g', xycoords='data',
                xytext=(signal.get_fs()/2+50, 0.025), textcoords='data', arrowprops=dict(arrowstyle="->", color='g'))
    ax.set_xlabel(r'$f$ (Hz)')
    ax.set_ylabel(r'$|X(f)|$')
    ax.grid()
    ax.legend()
    pyplot.savefig("../data/signal/signal2.png")
