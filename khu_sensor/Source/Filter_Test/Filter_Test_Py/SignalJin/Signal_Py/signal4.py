...
"""@package docstring
@brief	case 4 : signal with noise which SNR is 10dB
...
if __name__ == "__main__":
    ...
    """
    # create signal
    signal = SignalJin(0, 0.015, SNR=10, fs=1e3)
    ...
    ax.set(title="signal with noise (SNR = 10dB)")
    ...
    # cut the plot
    ax.axvline(x=500, color='g', linestyle='--')
    ax.annotate(rf'<-Nyquist Rate', xy=(510, 0.25), color='g')
    ax.set_xlim(0, 800)
    ...
    ax.annotate('Nyquist rate'+"\n"+r'$\frac{f_s}{2}$'+"={}kHz".format(signal.get_fs()/2e3),
                xy=(signal.get_fs()/2, 0.013), color='g', xycoords='data',
                xytext=(signal.get_fs()/2+50, 0.013), textcoords='data', arrowprops=dict(arrowstyle="->", color='g'))
    ax.set_xlabel(r'$f$ (Hz)')
    ax.set_ylabel(r'$|X(f)|$')
    ax.grid()
    ax.legend()
    pyplot.savefig("../data/signal/signal4.png")
