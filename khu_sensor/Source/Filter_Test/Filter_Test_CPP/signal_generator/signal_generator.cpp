...
: m_SNR(t_SNR) {
    if (t_Frequency == nullptr) {
        ...
        *this->m_Frequency = *t_Frequency; // deep copy
        ...
        if(this->m_Frequency->GetCount() > 1){
...
int signal_generator::reset(AVLTree<int> *t_Frequency, float t_SNR) {
    if (t_Frequency == nullptr) {
        return -1;
    } else {
        delete this->m_Frequency;
...
AVLTree<int> *signal_generator::getFrequency() const {
    return m_Frequency;
}

void signal_generator::setFrequency(AVLTree<int> *t_Frequency) {
